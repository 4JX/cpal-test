...
pub fn capture_output_audio(
    ...
    match audio_cfg.sample_format() {
        cpal::SampleFormat::F32 => match device.build_input_stream(
            &audio_cfg.config(),
            move |data, _info| wave_reader::<f32>(data, &mut f32_samples, &rms_sender),
            capture_err_fn,
        ) {
        ...
        cpal::SampleFormat::I16 => match device.build_input_stream(
            &audio_cfg.config(),
            move |data, _info| wave_reader::<i16>(data, &mut f32_samples, &rms_sender),
            capture_err_fn,
        ) {
            Ok(stream) => Some(stream),
            Err(_e) => None,
        ...
        cpal::SampleFormat::U16 => match device.build_input_stream(
            &audio_cfg.config(),
            move |data, _info| wave_reader::<u16>(data, &mut f32_samples, &rms_sender),
            capture_err_fn,
        ) {
            Ok(stream) => Some(stream),
            Err(_e) => None,
